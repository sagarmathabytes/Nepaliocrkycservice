...
interface ForeignStatusProps {
  data: KYCData;
  onNext: (data: Partial<KYCData>) => void;
  onBack: () => void;
}

export function ForeignStatus({ data, onNext, onBack }: ForeignStatusProps) {
  return (
    <form onSubmit={(e) => { e.preventDefault(); onNext({}); }} className="bg-white rounded-xl border border-gray-200 p-8">
      <div className="mb-6">
        <h2 className="text-gray-900 mb-2">Foreign / Special Status (Conditional)</h2>
        <p className="text-sm text-gray-600">For NRN, foreigners, or permanent residents</p>
      </div>
